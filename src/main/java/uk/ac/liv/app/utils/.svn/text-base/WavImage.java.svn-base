...
public class WavImage {
	...
	private void drawWaveForm(Graphics g, float frame_ref, float duration_time) {
		...
		if (samples != null) {
			...
			float lengthTime = duration_time;
	        ...
	        float increment = duration_time * sampleRate / width;
	        if (nbSamples > duration_time * sampleRate) {
	        	increment = nbSamples  / (float) width;	
	        } 
			...
			for (float t = 0 ; t < nbSamples ; t += increment) {		
				
				double scaledSample = samples.get((int)t) * yScaleFactor;
