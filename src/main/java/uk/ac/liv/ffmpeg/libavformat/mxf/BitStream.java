...
import uk.ac.liv.ffmpeg.libavcodec.mpeg12.MpegException;
import uk.ac.liv.ffmpeg.libavcodec.mpeg12.VLCTable;
...
public class BitStream {
     ...
     */
    public final int getVLC( VLCTable table ) throws MpegException {
        int code = show12Bits();
        int length = table.getCodeLength( code );
        if ( length > 0 ) {
            bitIndex += length;
            if (availableBits() < 0 ) throw new MpegException( "Buffer underflow" );
            return table.getCodeValue( code );
        } else if ( length < 0 ) {
            bitIndex += 12;
            return getVLC( table.getNextLevel( code ) );
        }
        if (availableBits() < 12 ) throw new MpegException( "Buffer underflow" );
        throw new MpegException( "Illegal VLC code " + table );
    }
}
